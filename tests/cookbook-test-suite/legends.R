...
save_outputs(temp, 'legends/hidden legend - 1')

# It can also be done when specifying the scale
temp <- bp + scale_fill_discrete(guide=FALSE)
save_outputs(temp, 'legends/hidden legend - 2')

# This removes all legends
temp <- bp + theme(legend.position="none")
save_outputs(temp, 'legends/hidden legend - all')
...
save_outputs(temp, 'legends/reversed legend')
...
save_outputs(temp, 'legends/removed legend title')

# Remove title for all legends
temp <- bp + theme(legend.title=element_blank())
save_outputs(temp, 'legends/removed legend titles')

temp <- bp + scale_fill_discrete(name="Experimental\nCondition")
save_outputs(temp, 'legends/multiple line legend title')

temp <- bp + scale_fill_discrete(name="Experimental\nCondition",
                         breaks=c("ctrl", "trt1", "trt2"),
                         labels=c("Control", "Treatment 1", "Treatment 2"))
save_outputs(temp, 'legends/legend title with custom labels')
...
temp <- bp + scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"),
                       ...
                       breaks=c("ctrl", "trt1", "trt2"),
                       labels=c("Control", "Treatment 1", "Treatment 2"))
save_outputs(temp, 'legends/legend title with custom colors')
...
temp <- lp + scale_shape_discrete(name  ="Payer",
                          ...
                          labels=c("Woman", "Man"))
save_outputs(temp, 'legends/line chart with custom legend')
...
save_outputs(temp, 'legends/legend in bottom left')
...
save_outputs(temp, 'legends/legend in bottom right')
