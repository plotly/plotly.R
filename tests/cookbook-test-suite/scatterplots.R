...
save_outputs(g, 'scatterplots/scatterplots with shading and regression line')

g <- ggplot(dat, aes(x=xvar, y=yvar)) +
    geom_point(shape=1) +    # Use hollow circles
    geom_smooth(method=lm,   # Add linear regression line
                se=FALSE)    # Don't add shaded confidence region
save_outputs(g, 'scatterplots/scatterplots without shading and regression line')


g <- ggplot(dat, aes(x=xvar, y=yvar)) +
    geom_point(shape=1) +    # Use hollow circles
    geom_smooth()            # Add a loess smoothed fit curve with confidence region
save_outputs(g, 'scatterplots/scatterplots shading and smoothed fit curve')

# Set color by cond
g <- ggplot(dat, aes(x=xvar, y=yvar, color=cond)) + geom_point(shape=1)
save_outputs(g, 'scatterplots/scatterplots colored by condition')
...
g <- ggplot(dat, aes(x=xvar, y=yvar, color=cond)) + geom_point(shape=1) +
    ...
    geom_smooth(method=lm,   # Add linear regression lines
                se=FALSE)    # Don't add shaded confidence region
save_outputs(g, 'scatterplots/scatterplots colored by condition with regression line')
...
save_outputs(g, 'scatterplots/scatterplots colored by condition with extended regression line')

# Set shape by cond
g <- ggplot(dat, aes(x=xvar, y=yvar, shape=cond)) + geom_point()
save_outputs(g, 'scatterplots/scatterplots shaped by condition')

# Same, but with different shapes
g <- ggplot(dat, aes(x=xvar, y=yvar, shape=cond)) + geom_point() +
    scale_shape_manual(values=c(1,2))  # Use a hollow circle and triangle
save_outputs(g, 'scatterplots/scatterplots hollow shapes by condition')
...
save_outputs(g, 'scatterplots/scatterplots with overlapped points')
...
save_outputs(g, 'scatterplots/scatterplots with jittered points')
