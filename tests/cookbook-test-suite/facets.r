...
save_outputs(sp, 'facets/no features')

# Divide by levels of "sex", in the vertical direction
sp1 <- sp + facet_grid(sex ~ .)
save_outputs(sp1, 'facets/vertical facet_grid')

# Divide by levels of "sex", in the horizontal direction
sp2 <- sp + facet_grid(. ~ sex)
save_outputs(sp2, 'facets/horizontal facet_grid')
...
save_outputs(sp4, 'facets/facet_wrap')

sp5 <- sp + facet_grid(sex ~ day) +
  theme(strip.text.x = element_text(size=8, angle=75),
        strip.text.y = element_text(size=12, face="bold"),
        strip.background = element_rect(colour="red", fill="#CCCCFF"))
save_outputs(sp5, 'facets/facet label colors')
...
save_outputs(sp6, 'facets/custom facet labels')
...
save_outputs(sp8, 'facets/histogram facets with free_y')
