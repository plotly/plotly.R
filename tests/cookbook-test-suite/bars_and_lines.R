...
save_outputs(g, 'bars-and-lines/basic-bar-fill-colors-1')
g <- ggplot(data=df, aes(x=time, y=total_bill)) + geom_bar(aes(fill=time), stat="identity")
save_outputs(g, 'bars-and-lines/basic-bar-fill-colors-2')

# Add a black outline
g <- ggplot(data=df, aes(x=time, y=total_bill, fill=time)) + geom_bar(colour="black", stat="identity")
save_outputs(g, 'bars-and-lines/basic-bar-fill-colors-black-outline')

# No legend, since the information is redundant
g <- ggplot(data=df, aes(x=time, y=total_bill, fill=time)) +
    geom_bar(colour="black", stat="identity") +
    guides(fill=FALSE)
save_outputs(g, 'bars-and-lines/basic-bar-fill-colors-black-outline-no-legend')
...
save_outputs(g, 'bars-and-lines/basic-bar-fully-styled')
...
save_outputs(g, 'bars-and-lines/basic-dashed-line-with-colors')
...
g <- ggplot(data=df, aes(x=time, y=total_bill, group=1)) + geom_line() + geom_point() +
    ...
    xlab("Time of day") + ylab("Total bill") +
    ggtitle("Average bill for 2 people")
save_outputs(g, 'bars-and-lines/basic-line-fully-styled')
...
save_outputs(g, 'bars-and-lines/basic-line-with-mapped-colors')
...
save_outputs(g, 'bars-and-lines/basic-line-with-symbols')
...
save_outputs(g, 'bars-and-lines/basic-line-with-points-fully-styled')

g <- ggplot(data=df1, aes(x=sex, y=total_bill, group=time, shape=time, color=time)) + geom_line() + geom_point()
save_outputs(g, 'bars-and-lines/basic-line-swapped-vars')
...
g <- ggplot(data=df1, aes(x=time, y=total_bill, fill=sex)) +
    ...
    theme_bw()
save_outputs(g, 'bars-and-lines/finished-bar-bw-theme')
...
save_outputs(g, 'bars-and-lines/finished-line')
...
save_outputs(g, 'bars-and-lines/line-continuous-numerical-x-axis')
...
save_outputs(g, 'bars-and-lines/line-continuous-categorical-x-axis')
...
## save_outputs(g, 'bars-and-lines/line-continuous-categorical-x-axis-with-factor')

# Use dfn2 from above
g <- ggplot(data=dfn2, aes(x=dose, y=length, fill=supp)) + geom_bar(stat="identity", position=position_dodge())
save_outputs(g, 'bars-and-lines/bar-categorical-numerical-labels')

# Use the original data frame, but put factor() directly in the plot specification
g <- ggplot(data=dfn, aes(x=factor(dose), y=length, fill=supp)) + geom_bar(stat="identity", position=position_dodge())
save_outputs(g, 'bars-and-lines/bar-categorical-numerical-labels-with-factor')
