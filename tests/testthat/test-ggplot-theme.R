...
test_that("background translated correctly",{
  ...
  info <- expect_doppelganger(ggiris, "theme-background")
  L <- info$layout
...
test_that("grid/ticks translated correctly",{
  ...
  info <- expect_doppelganger(ggiris, "theme-ticks-and-grids")
  for (xy in c("x", "y")) {
...
test_that("show ticks as 'outside' by default", {
  ggiris <- iris.base
  info <- expect_doppelganger(ggiris, "theme-ticks-default")
  for (xy in c("x", "y")) {
    ax.list <- info$layout[[paste0(xy, "axis")]]
...
test_that("do not show zeroline by default", {
  ggiris <- iris.base
  info <- expect_doppelganger(ggiris, "theme-zeroline-default")
  for (xy in c("x", "y")) {
    ax.list <- info$layout[[paste0(xy, "axis")]]
...
test_that("marker default shape is a circle", {
  info <- expect_doppelganger(gg, "theme-marker-default")
  for (i in c(1:3)) {
...
test_that("plot panel border is translated correctly", {
  ...
  info <- expect_doppelganger(ggiris, "theme-panel-border-1")
  ...
  info <- expect_doppelganger(red, "theme-panel-border-2")
